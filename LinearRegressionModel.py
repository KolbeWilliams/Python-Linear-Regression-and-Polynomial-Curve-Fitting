...
y = np.array(y_values)
try:
    range_min = float(input('Enter the minimum value you want to test for the slope and intercept of the regression line: '))
    range_max = float(input('Enter the maximum value you want to test for the slope of the regression line: '))
    step = float(input('Enter the step for the slopes and intercepts that you want to test: '))
    if step <= 0 or step <= 0:
...
slopes = np.arange(range_min, range_max, step)
intercepts = np.arange(range_min, range_max, step)
...
surf = ax.plot_surface(X, Y, Z, alpha = 0.75, cmap = 'viridis') #Plot the graph
fig.colorbar(surf, shrink = 0.5, aspect = 20, location = 'left')
